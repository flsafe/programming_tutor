...
# modify to describe the One Thousand Lines Of Code badge.

describe OneThousandLinesOfCode do
  before(:each) do
    @badge = OneThousandLinesOfCode.new
  end

  describe "#has_earned?" do
    it "awards the badge if the stats has at least 1KLOC" do
      stats = stub_model(StatsSheet, :loc_count => 2000)
      @badge.has_earned?(stats).should == true    
    end
    it "does not award the badge if the total xp is == 0" do
      stats = stub_model(StatsSheet, :loc_count => 500)
