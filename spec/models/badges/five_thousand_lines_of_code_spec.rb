...
# modify to describe the Five Thousand Lines Of Code badge.

describe FiveThousandLinesOfCode do
  before(:each) do
    @badge = FiveThousandLinesOfCode.new
  end

  describe "#has_earned?" do
    it "awards the badge if the stats has at least 1 xp" do
      stats = stub_model(StatsSheet, :loc_count => 5000)
      @badge.has_earned?(stats).should == true    
    end
    it "does not award the badge if the total xp is == 0" do
      stats = stub_model(StatsSheet, :loc_count => 2500)
